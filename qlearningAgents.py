...
class ApproximateQAgent(PacmanQAgent):
    ...
    def writeWeights2CSV(self):
        import pandas as pd
        df = pd.DataFrame(self.weight_history)
        # write to csv with index (episode number in multiples of 10)
        # df.index = pd.Index(range(10, len(self.weight_history) * 10 + 10, 10), name='episode')
        df.index = pd.Index(range(1, len(self.weight_history) + 1), name='episode')
        df.to_csv('weights/weights.csv', index=True, header=True)
